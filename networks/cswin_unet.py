...
class SkipAttention(nn.Module):
    ...
    def forward(self,decoder_feat,encoder_feat):
        ...
        H=W=int(L**0.5)
        dec = decoder_feat.transpose(1,2).view(B,C,H,W)
        enc = encoder_feat.transpose(1,2).view(B,C,H,W)

        # fuse = torch.cat([dec,enc], dim=1)
        gate=self.attn(enc)
        out = dec+gate*enc
...
class CSWinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=8, embed_dim=64, depth=[1, 2, 9, 1],
        ...
        self.skip_attn3 = SkipAttention(embed_dim * 4)
        self.skip_attn2 = SkipAttention(embed_dim * 2)
        self.skip_attn1 = SkipAttention(embed_dim)

        H1, W1 = img_size // 4, img_size // 4  # 56*56
        self.pos_embed_2d = nn.Parameter(torch.zeros(1, embed_dim, H1, W1))
    ...
    def forward_features(self, x):
        x = self.stage1_conv_embed[0](x)
        x = x + self.pos_embed_2d.to(x.device)
        x = self.stage1_conv_embed[1](x)
